...
async def create_message(room_id: int, message_data: Annotated[MessageCreate, Depends()],
                         session: AsyncSession = Depends(get_async_session)) -> MessageRead:
    new_message = Message(content=message_data.content, user_id=message_data.user_id,
                          room_id=room_id)
